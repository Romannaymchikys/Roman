...
public:
	...
	void aCapMenu(std::string functional(std::string&, std::string&, 
		                                 ...
		                                 const std::vector<char>&,
		                                 std::vector<size_t>& );
	// запускает две  асинхронные функции(возможно, в отдельном потоке) для
	// измерения количество тиков
	static bool RefinementData(const std::string&, const std::string&);
	//функция которая считает количество тиков
	static std::chrono::nanoseconds Sorted( std::string);
